...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num_dice in range(1, 11):
        ...
        score = averaged_roll_dice(num_dice, dice)
        if score > max_score:
            max_score = score
...
def final_strategy(score, opponent_score):
    ...
    1.优先考虑最稳定的free_bacon规则,如果这份稳定可以给我们带来足够的收益,那么直接选择投0个骰子。
    2.我们知道,在游戏规则中,一旦投出任一一个1点,那么总得分就为1,这看似是一个劣势,但在特殊情况下也可以是优势。
    因为这个"1分"也是一个稳定项。如果我们获得1分后可以获得一个额外回合,那我们可以直接选择投10个骰子,大概率获
    得我们希望的1分,从而进行额外回合。而如果获得的不是1分,那就再好不过了,10个骰子的分数足以远远甩开对手。
    所以该策略总是正面收益。
    3.当上述两个稳定的策略都不能带来可观收益时,我们便正常投骰子。一次投6个骰子的时候可以获得最大的数学期望,
    但数学期望最大并不代表这就是最优解。当分数显著高于对手时,投更少的骰子反而可以稳固自己的优势;
    相应的,分数显著低于对手时则要考虑投更多的骰子以期望追上对手。
    """
    # BEGIN PROBLEM 12
    cutoff = 8
    num_rolls = 6
    ...
    #当使用free_bacon规则收益足够大，或可以获得额外回合时，选择投0个
    if extra_turn(potential_score, opponent_score) or bacon_score >= cutoff:
        return 0
    #当用free_bacon规则可以直接获胜时候就投0个
